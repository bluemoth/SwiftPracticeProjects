...
class ViewController: UICollectionViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let savedPeople = defaults.object(forKey: "people") as? Data {
            if let decodedPeople = try? NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(savedPeople) as? [Person] {
                ...
                print("loaded items from userDefaults \(people)")
            }
        }
        
        // add button on left to allow user to select an image within directory
        navigationItem.leftBarButtonItem = UIBarButtonItem(barButtonSystemItem: .add, target: self, action: #selector(addNewPerson))
        navigationItem.rightBarButtonItem = UIBarButtonItem(title: "Camera", style: .plain, target: self, action: #selector(takePicture))
        
        // block to check what's actually in defaults
        let domain = Bundle.main.bundleURL
        print(domain)
    ...
    // saving current state of people to userDefaults any time function called
    func save() {
        if let savedData = try? NSKeyedArchiver.archivedData(withRootObject: people, requiringSecureCoding: false) {
            let defaults = UserDefaults.standard
            defaults.set (savedData, forKey: "people")
        }
        print("saving \(people) to defaults")
    }
}
